...
public class OrderServiceImpl implements OrderService {
    ...
    public void userCancelById(Long id) throws Exception {
        ...
        if(db==null) throw new OrderBusinessException(MessageConstant.ORDER_NOT_FOUND);
        if(db.getStatus()>2) throw new OrderBusinessException(MessageConstant.ORDER_STATUS_ERROR);
    ...
    public void repetition(Long id) {
        Long userId = BaseContext.getCurrentId();
        List<OrderDetail> detailList = orderDetailMapper.getByOrderId(id);
        List<ShoppingCart> cartList = detailList.stream().map(d->{
            ShoppingCart sc = new ShoppingCart();
            BeanUtils.copyProperties(d,sc,"id");
            sc.setUserId(userId);
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO dto) {
        ...
        for(Orders o : page.getResult()){
            ...
            BeanUtils.copyProperties(o,vo);
            List<OrderDetail> details = orderDetailMapper.getByOrderId(o.getId());
            String disStr = details.stream().map(x->x.getName()+"*"+x.getNumber()+";").collect(Collectors.joining(""));
            vo.setOrderDishes(disStr);
            voList.add(vo);
        }
        return new PageResult(page.getTotal(),voList);
    }
    ...
    public void confirm(OrdersConfirmDTO dto) {
        Orders orders = Orders.builder().id(dto.getId()).status(Orders.CONFIRMED).build();
        orderMapper.update(orders);
    }
